...
const agePracticeMap = {
  ...
  5: ['ಕ', 'ಕಾ', 'ಕಿ','ಕೀ','ಕು','ಕೂ','ಕೆ','ಕೇ','ಕ್','ಕೈ','ಕೊ','ಕೋ','ಕೌ','ಕಂ', 'ಕಃ',
  ...
  7: ['ರ', 'ರಾ', 'ರಿ', 'ರೀ', 'ರು', 'ರೂ', 'ರೃ', 'ರೆ', 'ರೇ', 'ರೈ', 'ರೊ', 'ರೋ', 'ರೌ', 'ರಂ', 'ರಃ',
...
async function initSodaReport() {
  ...
  if (Array.isArray(sodaResults) && sodaResults.length) {
    ...
    if (sodaFresh || signature !== lastSig) {
      ...
      try {
        await fetch(`http://localhost:3000/api/children/${user.id}/report`, {
          method: 'POST',
...
function summarizeSODA(results = []) {
  const summary = { Correct: 0, Substitution: 0, Omission: 0, Addition: 0, Distortion: 0 };
...
async function loadReportHistory(childId) {
  const res = await fetch(`http://localhost:3000/api/children/${childId}/reports`);
  if (!res.ok) return;
  const reports = await res.json();
  if (!Array.isArray(reports) || !reports.length) return;

  const progressLabels = reports.map(r => new Date(r.date).toLocaleDateString());
  const progressData = reports.map(r => overallFromSummary(r.summary || summarizeSODA(r.sodaResults || [])));
  ...
  if (ctx) {
    ...
    window.progressChartInstance = new Chart(ctx, {
      type: 'line',
      data: { labels: progressLabels, datasets: [{ label: 'ಒಟ್ಟು ಸರಿಯಾದ ಉಚ್ಚಾರಣೆ (%)', data: progressData, fill: true }] },
  ...
  if (historyDiv) {
    historyDiv.innerHTML =
      '<h4>ಹಳೆಯ ವರದಿಗಳು</h4>' +
      reports.map((r, i) => `<button class="history-btn" data-idx="${i}">${progressLabels[i]}</button>`).join(' ');
...
function buildReportDetails(report) {
  ...
  const correctList = [...new Set(
    correctResults.map(r => r.target_word).filter(Boolean)
  )];

  // ---------- Wrong list ----------
  const wrongList = [...new Set(
    errorResults.map(r => r.target_word).filter(Boolean)
  )];
  ...
  const uniquePractice = [...new Set(practiceWords)];
...
function showReport(report) {
  ...
  const correctResults = (report.sodaResults || [])
  .filter(r => r.error_type == "");

  // Extract error WORDS from error results (fallback to words, as requested)
  const errorResults = (report.sodaResults || [])
    .filter(r => r.error_type || (r.distortion_score ?? 0) > 0);
  ...
  const details = buildReportDetails(report);

  document.getElementById('correct-list').textContent =
    details.correctList.length ? details.correctList.join(', ') : 'ಯಾವು ಕಂಡುಬಂದಿಲ್ಲ';

  document.getElementById('wrong-list').textContent =
    details.wrongList.length ? details.wrongList.join(', ') : 'ಯಾವುದೇ ದೋಷಗಳು ಕಂಡುಬಂದಿಲ್ಲ';


  // document.getElementById('wrong-letter-list').textContent =
  //   details.uniquePractice.length ? details.uniquePractice.join(', ') : '';
  
  document.getElementById('practice-list').textContent =
    details.practiceList.length ? details.practiceList.join(', ') : '';
...
async function downloadReport() {
  ...
  const errorResults = (latestReport.sodaResults || [])
    .filter(r => r.error_type || (r.distortion_score ?? 0) > 0);
  const practiceWords = errorResults
    .map(r => r.target_word)
    .filter(w => w && String(w).trim().length > 0);

  const details = buildReportDetails(latestReport);
