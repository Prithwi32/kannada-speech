...
function displayProfile(child) {
  ...
  document.getElementById('genderValue').textContent = getGenderDisplay(child.ಲಿಂಗ);
...
function getGenderDisplay(gender) {
  switch (gender) {
    case 'Male': return 'ಪುರುಷ';
    case 'Female': return 'ಹೆಣ್ಣು';
    case 'Other': return 'ಇತರ';
...
async function saveProfile() {
  ...
  try {
    ...
    if (!updatedData.ಹೆಸರು || !updatedData.ವಯಸ್ಸು || !updatedData.ಲಿಂಗ || !updatedData.ಪೋಷಕರು || !updatedData.ನಗರ || !updatedData.ಇಮೇಲ್) {
      showError('ದಯವಿಟ್ಟು ಎಲ್ಲಾ ಅಗತ್ಯ ಕ್ಷೇತ್ರಗಳನ್ನು ಭರ್ತಿ ಮಾಡಿ');
      return;
    }

    // Update in database
    const response = await fetch(`http://localhost:3000/api/children/${user.id}`, {
      method: 'PUT',
      headers: {
